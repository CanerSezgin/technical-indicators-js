...
export const getTypicalPrice = (price: StockPrice): number => (price.high + price.low + price.close) / 3;

export const getPriceChange = (oldPrice: number, newPrice: number): GainLoss => {
  let status;
